...
export class WorldSimulator {
  ...
  static async simulateNPCBattles(playerBattleCount: number = 1,char: Character): Promise<{
      ...
      for (let i = 0; i < npcBattlesToSimulate; i++) {
        ...
        if (battleResult) {
          ...
          if (battleResult.legendFell) {
            ...
            results.summary.push(`💀 ${battleResult.loser.name} foi derrotado por ${battleResult.winner.name}!`);
          }
          
          // Adicionar eventos interessantes
          if (battleResult.bountyGain > 100000000) { // 100M+
            results.summary.push(`💰 ${battleResult.winner.name} ganhou ${this.formatBounty(battleResult.bountyGain)} de bounty!`);
          }
        }
  ...
  private static async findNPCOpponents(char: Character): Promise<[Character | null, Character | null]> {
    try {
      // Buscar NPCs ativos (não é jogador)
      const allNPCs = await db.characters
    .where('isPlayer')
    .equals(0)
    .toArray();
    ...
    const filteredNPCs = allNPCs.filter(npc => npc.id !== char.id);
      ...
      const potentialOpponents = filteredNPCs.filter(npc => 
        npc.id !== fighter1.id &&
        npc.level >= levelRange.min &&
        npc.level <= levelRange.max
      );
      
      if (potentialOpponents.length === 0) {
        // Se não encontrar no range, expandir busca
        const expandedOpponents = filteredNPCs.filter(npc => 
          npc.id !== fighter1.id &&
          Math.abs(npc.level - fighter1.level) <= 10
        );
      ...
      const fighter2 = prioritizedOpponents[Math.floor(Math.random() * Math.min(prioritizedOpponents.length, 5))];
  ...
  private static prioritizeOpponents(fighter: Character, opponents: Character[]): Character[] {
    return opponents.sort((a, b) => {
      ...
      if ((fighter.type === 'Pirate' && a.type === 'Marine') || 
          (fighter.type === 'Marine' && a.type === 'Pirate')) {
        scoreA += 5;
      }
      if ((fighter.type === 'Pirate' && b.type === 'Marine') || 
  ...
  private static async simulateBattle(fighter1: Character, fighter2: Character): Promise<{
    winner: Character;
    loser: Character;
  } | null> {
  ...
  private static calculateNPCBattles(playerBattles: number): number {
    ...
    const dataSize = 200
    const baseNPCBattles = playerBattles * this.randomBetween(dataSize * 0.4,dataSize);
    
    // Adicionar variação baseada no tempo
    const timeVariation = this.randomBetween(10, 50);
    
    return Math.max(baseNPCBattles + timeVariation, dataSize * 0.5); // Máximo 50 batalhas por vez
  }
  
  // 11. 💾 SALVAR REGISTRO DE BATALHA
  private static async saveBattleRecord(winner: Character, loser: Character, expGain: number, bountyGain: number): Promise<void> {
    try {
