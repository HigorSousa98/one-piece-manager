...
export class AvatarGenerationSystem {
  ...
  private static async generateComponents(character: Character): Promise<AvatarComponents> {
    ...
    const devilFruit = character.devilFruitId
      ? await db.devilFruits.get(character.devilFruitId)
      : null
  ...
  // ✅ GERAR ROUPAS
  private static generateClothing(
    character: Character,
    seed: number,
  ): AvatarComponents['clothing'] {
  ...
  // ✅ GERAR BACKGROUND
  private static generateBackground(
    character: Character,
    seed: number,
  ): AvatarComponents['background'] {
  ...
  private static renderBackground(
    background: AvatarComponents['background'],
    width: number,
    height: number,
  ): string {
    let bgSvg = ''
  ...
  private static renderFaceAndNeck(
    face: AvatarComponents['face'],
    width: number,
    height: number,
  ): string {
  ...
  private static renderAnimeEyes(
    eyes: AvatarComponents['eyes'],
    width: number,
    height: number,
  ): string {
    const centerX = width / 2
  ...
  private static renderDetailedAnimeEye(
    x: number,
    y: number,
    eyes: AvatarComponents['eyes'],
    side: 'left' | 'right',
  ...
  private static renderAnimeMouth(
    expression: AvatarComponents['face']['expression'],
    width: number,
    height: number,
  ): string {
    const centerX = width / 2
  ...
  private static renderHairBack(
    hair: AvatarComponents['hair'],
    width: number,
    height: number,
  ): string {
  ...
  private static renderHairFront(
    hair: AvatarComponents['hair'],
    width: number,
    height: number,
  ): string {
  ...
  private static generateAfroTexture(
    centerX: number,
    centerY: number,
    radius: number,
    color: string,
  ...
  private static renderAnimeClothing(
    clothing: AvatarComponents['clothing'],
    width: number,
    height: number,
  ): string {
    const centerX = width / 2
  ...
  private static renderFaceScars(
    scars: AvatarComponents['scars'],
    width: number,
    height: number,
  ): string {
  ...
  private static renderDevilFruitSymbol(
    devilFruit: AvatarComponents['devilFruit'],
    width: number,
    height: number,
  ): string {
  ...
  private static async getExistingAvatar(characterId: number): Promise<Avatar | null> {
    try {
      const avatar = await db.avatars.where('characterId').equals(characterId).first()
  ...
  static async regenerateAvatar(characterId: number): Promise<GeneratedAvatar | null> {
    try {
      ...
      await db.avatars.where('characterId').equals(characterId).delete()
