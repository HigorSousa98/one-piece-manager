...
export class IslandExplorationSystem {
  ...
  static async completeTask(taskId: number): Promise<{
    ...
    try {
      ...
      const crewMembers = await db.characters
        .where('crewId')
        .equals(character.crewId)
        .and((char) => char.id != character.id)
        .toArray()
      ...
      const canRecruit =
        (civilian &&
          ...
          Math.random() < GenerationConfig.createEpic().civillianRecruitmentChance &&
          (await RecruitmentSystem.validateTargetCrewSize(
            character.crewId,
            GenerationConfig.createEpic(),
          ))) ||
        false
  ...
  static async getTaskStatistics(characterId: number): Promise<{
    ...
    const totalKindnessGained = completedTasks.reduce(
      (sum, task) => sum + (task.kindnessReward || 0),
      0,
    )
    const totalExperienceGained = completedTasks.reduce(
      (sum, task) => sum + (task.experienceReward || 0),
      0,
    )
