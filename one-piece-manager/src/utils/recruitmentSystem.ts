...
export class RecruitmentSystem {
  ...
  public static async validateTargetCrewSize(
    targetCrew: number | undefined,
    generator: GenerationConfig,
  ): Promise<boolean> {
    const crewMembers = targetCrew
      ? await db.characters.where('crewId').equals(targetCrew).toArray()
      : null
    const ship = targetCrew
      ? await db.ships.where('crewId').equals(targetCrew).limit(1).toArray()
      : null
    if (crewMembers && ship) {
  ...
  private static calculateRecruitmentChance(
    recruiter: Character,
    target: Character,
  ): {
    chance: number
  ...
  static async attemptRecruitment(
    ...
    if (success) {
      ...
      if (actualCrew) {
        const characterOnCrew = characters.filter(
          (char) => char.crewId === actualCrew.id && char.id != target.id,
        )
