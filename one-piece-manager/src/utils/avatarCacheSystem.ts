...
export class AvatarCacheSystem {
  ...
  private static async shouldEvict(newEntry: CacheEntry): Promise<boolean> {
    ...
    return currentSize + newEntry.size > this.MAX_CACHE_SIZE || currentEntries >= this.MAX_ENTRIES
  }
  ...
  static async getStats(): Promise<CacheStats> {
    ...
    const averageAccessTime =
      this.cacheStats.hits > 0 ? this.cacheStats.totalAccessTime / this.cacheStats.hits : 0
