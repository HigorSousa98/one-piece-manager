...
export class BossFightSystem {
  ...
  static async startBossFight(
    playerCrewId: number, 
    detectedBoss: DetectedBoss
  ): Promise<number> {
    try {
      // ✅ VERIFICAR SE JÁ EXISTE COMBATE ATIVO
      const existingFight = await db.bossFights
        .where('playerCrewId').equals(playerCrewId)
  ...
  static async getAvailablePlayerMembers(bossFightId: number): Promise<Character[]> {
    ...
    const allMembers = await db.characters
      .where('crewId').equals(bossFight.playerCrewId)
      .toArray()
    
    // ✅ FILTRAR MEMBROS DERROTADOS
    return allMembers.filter(member => 
      !bossFight.playerDefeatedMembers.includes(member.id!)
    )
  }
  ...
  static async updateBossFightAfterBattle(
    ...
    if (playerWon) {
      ...
      if (availableMembers.length <= 1) { // -1 porque o atual acabou de perder
        updates.isCompleted = true
  ...
  private static async processBossDefeat(bossFight: BossFight, playerCrewId: number): Promise<void> {
    try {
      ...
      const crewMembers = await db.characters
        .where('crewId').equals(playerCrewId)
        .toArray()
      ...
      // ✅ FINALIZAR TASK
      await db.tasks
        .where('targetId').equals(bossFight.id!)
  ...
  private static async processPlayerDefeat(bossFight: BossFight): Promise<void> {
    try {
      ...
      const escapeIslands = await db.islands
        .where('difficulty').equals(targetDifficulty)
        .toArray()
