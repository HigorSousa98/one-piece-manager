...
export class WorldEncyclopedia {
  ...
  static async generateWorldRankings(playerCharacterId?: number): Promise<WorldRankings> {
    try {
      // Buscar dados base
      const [characters, crews, islands, yonkous, shichibukais, admirals, gorouseis] = await Promise.all([
   ...
   */
  static async getRankingByCategory(category: string, playerCharacterId?: number): Promise<RankingCharacter[]> {
    const rankings = await this.generateWorldRankings(playerCharacterId)
    
    switch (category) {
      case 'yonkou': return rankings.yonkou
      case 'shichibukai': return rankings.shichibukai
      case 'admirals': return rankings.admirals
      case 'gorousei': return rankings.gorousei
      case 'pirates': return rankings.pirates
      case 'marines': return rankings.marines
      case 'bountyHunters': return rankings.bountyHunters
      case 'government': return rankings.government
      case 'supernovas': return rankings.supernovas
      default: return []
    }
  }
  ...
  static async getWorldStats(): Promise<{
    ...
    const allBounties = [
      ...
      ...rankings.government
    ].map(c => c.bounty || 0).filter(b => b > 0)

    const highestBounty = Math.max(...allBounties, 0)
    const averageBounty = allBounties.length > 0 ? allBounties.reduce((a, b) => a + b, 0) / allBounties.length : 0
